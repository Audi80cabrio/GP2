 ...
 * 4.0.3:
 * - console output changed
 * - implicit type conversions eliminated: ftime = ftime + 0.1f;
 *   vel = (float)v / 3.6f; dist = dist + vel / 10.0f; 
 *   x = x + vel / 10.0f * xvect; y = y + vel / 10.0f * yvect;
 *   z = z + vel / 10.0f * zvect; float distToDest = 
